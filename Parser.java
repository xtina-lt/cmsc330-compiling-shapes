...
class Parser {
    ...
    /*  --------------------------------------------------------
    *                   IMPORTANT
    *             Add each new shap here.
    *  Add new tokens(that are needed for shape) to Token.java
    *    Change: Parser Tokens, added shape
    */
    private void parseImages(Scene scene, Token imageToken) throws LexicalError, SyntaxError, IOException {
        int height, width, offset, radius, side;
        String text;
        ...
        if (imageToken == Token.RIGHT_TRIANGLE) {
            ...
            int[] arr = getNumberList(2);
            Point pointB = new Point(arr[0], arr[1]);
            verifyNextToken(Token.OFFSET);
            verifyNextToken(Token.NUMBER);
            offset = lexer.getNumber();
            scene.addImage( new Parallelogram(color, offset, new Point[] {point, pointB}) );
        } else if (imageToken == Token.REGULAR_POLYGON) {
            verifyNextToken(Token.SIDES);
            verifyNextToken(Token.NUMBER);
            side = lexer.getNumber();
            verifyNextToken(Token.RADIUS);
            verifyNextToken(Token.NUMBER);
            radius = lexer.getNumber();
            scene.addImage( new RegularPolygon(color, side, point, radius) );
            ...
            verifyNextToken(Token.NUMBER);
            width = lexer.getNumber();
            scene.addImage( new Isosceles(color, height, width, point) );
        } else if (imageToken == Token.TEXT) {
            token = lexer.getNextToken();
            text = lexer.getLexeme();
            scene.addImage( new Text(color, text, point) );
        } else {
